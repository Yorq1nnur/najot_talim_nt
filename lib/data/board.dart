...
import 'package:najot_talim_nt/data/point.dart';
...
class Board extends Serializable {
  ...
  void serialize(SerializeOutput output) {
    ...
    output.writeSerializable(PointSerializableWrapper(blank) as Serializable);

    for (final chip in chips) {
      output.writeSerializable(chip as Serializable);
    }
  }
}
...
class BoardDeserializableFactory extends DeserializableHelper<Board> {
  ...
  Board? deserialize(SerializeInput input) {
    ...
    final blank = input.readDeserializable(pointFactory as DeserializableHelper);
    if (blank == null) {
      return null;
    }
    ...
    for (var i = 0; i < length; i++) {
      final chip = input.readDeserializable(chipFactory as DeserializableHelper);
      if (chip == null) {
        return null;
      }
